...
export default async function handler(req, res) {
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization, New-Api-User, X-Target-BaseUrl, X-Cubence-Token');
  ...
  try {
    ...
    // 特殊处理：Cubence API 使用 Cookie 认证
    // 如果有 X-Cubence-Token 头，构建 Cookie 头
    if (req.headers['x-cubence-token']) {
      forwardHeaders['cookie'] = `token=${req.headers['x-cubence-token']}`;
      console.log('[Proxy] 为 Cubence API 构建 Cookie 头');
    }
